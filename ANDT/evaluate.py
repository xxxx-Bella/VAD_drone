...
class GroundTruthLoader(object):
    ...
    NAME_MAT_MAPPING = {
        ...
        CAMPUS: os.path.join(DATA_DIR, 'Campus/playground_original/playground.mat'),
        CAMPUS_DRONE: os.path.join(DATA_DIR, 'Campus_Drone/playground_drone_original/playground_drone.mat'),
    ...
    NAME_FRAMES_MAPPING = {
        ...
        CAMPUS: os.path.join(DATA_DIR, 'Campus/playground_original/test'),
        CAMPUS_DRONE: os.path.join(DATA_DIR, 'Campus_Drone/playground_drone_original/test'),
    ...
    def __load_ucsd_avenue_DA_C_gt(self, dataset):  # ucsd ped1, ucsd ped2
        ...
        return abnormal_events  # load (DA or Campus or shanghai_Campus) gt: abnormal_events; oriiginal gt
...
def precision_recall_f1_auc(loss_file):
    print('\n--- compute precision_recall_f1_auc')
    ...
    for sub_loss_file in loss_file_list:
        ...
        if os.path.isdir(loss_file):
            # print(results)
            print('dataset = {}, loss_file = {}\nauc = {}, precious = {}, recall = {}, f1 = {}'.format(
                results.dataset,
                results.loss_file,
                results.auc,
                results.precious,
...
def calculate_score(loss_file):
    print('\n--- compute score')
    # ################## raw
#     if not os.path.isdir(loss_file):  # 1 model
#         loss_file_path = loss_file
#     else:                             # more than 2 models
#         optical_result = compute_auc(loss_file)
...
#           'delta = {} \n'.format(mean_normal_scores, mean_abnormal_scores, mean_normal_scores - mean_abnormal_scores))
    ...
    for file in os.listdir(loss_file):
        ...
        mean_normal_scores = np.mean(scores[labels == 0])
        mean_abnormal_scores = np.mean(scores[labels == 1])
        print('loss_file = {}\nmean normal scores = {}, mean abnormal scores = {}, delta = {}'.format(
              file,
              mean_normal_scores, 
              mean_abnormal_scores, 
